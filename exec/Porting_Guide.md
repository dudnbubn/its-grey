# It's Grey Porting Guide

## 0. Index

- [1. Deployment Settings](#1.-deployment-settings)
  - [1.1 NGINX 환경 설정](#1.1-nginx-환경-설정)
  - [1.2 Docker Compose 설정](#1.2-docker-compose-설정)
  - [1.3 Jenkins 설정](#1.3-jenkins-설정)
...
- Jenkins Shell Command (Build Steps)
  ...
  if [ "$new_container" == "its-grey-blue" ]; then
    docker --tlsverify --tlscacert=/.docker/ca.pem --tlscert=/.docker/cert.pem --tlskey=/.docker/key.pem -H={server_ip}:{port_number} exec nginx sed -i '4s/.*$/        server its-grey-blue:8000;/' /etc/nginx/sites-available/default
  elif [ "$new_container" == "its-grey-green" ]; then
    docker --tlsverify --tlscacert=/.docker/ca.pem --tlscert=/.docker/cert.pem --tlskey=/.docker/key.pem -H={server_ip}:{port_number} exec nginx sed -i '4s/.*$/        server its-grey-green:8000;/' /etc/nginx/sites-available/default
  fi
  
  # nginx reload
  docker --tlsverify --tlscacert=/.docker/ca.pem --tlscert=/.docker/cert.pem --tlskey=/.docker/key.pem -H={server_ip}:{port_number} exec nginx nginx -s reload
